 ...
 */
@Service("smartnotifier.ProcessPatientsOnArtWithHighViralLoadUseCase")
public class ProcessPatientsOnArtWithHighViralLoadService extends BaseOpenmrsService implements ProcessPatientsOnArtWithHighViralLoadUseCase {
	
	private PatientNotificationPort patientNotificationPort;
	
	@Autowired
	public ProcessPatientsOnArtWithHighViralLoadService(final PatientNotificationPort patientNotificationPort) {
	...
	public List<PatientNotification> process(final LocalDate endDate, final Location location) throws BusinessException {
		...
		for (final PatientNotification patientNotification : patientsToNotify) {
			...
			patientNotification.setNotificationType(NotificationType.ON_ART_WITH_HIGH_VIRAL_LOAD);
