   select coorte12meses_final.patient_id, identifier.identifier, coorte12meses_final.data_inicio, contact.value, coorte12meses_final.data_usar                                                                               
            ...
            (select inicio.*,                                                                                                                    
             ...
             INNER JOIN
             (
                SELECT last_viral_load_value.patient_id FROM
                (
                ...
                WHERE (TIMESTAMPDIFF(MONTH, last_viral_load_date.last_viral_load_date, :endDate) >= 12 
                    OR (TIMESTAMPDIFF(MONTH, last_viral_load_date.last_viral_load_date, :endDate) >= 3 AND last_viral_load_value >= 1000))
             )elligible_to_vl_collection ON elligible_to_vl_collection.patient_id = coorte12meses_final.patient_id
             LEFT JOIN
             (
			 ...
			 AND DATEDIFF( :endDate , data_inicio) >= 180 AND pregnant_and_breastfeeding.patient_id IS NULL