...
SELECT next_appointment.patient_id, identifier.identifier, start_art.art_start_date, contact.value, MAX(next_appointment_date) next_appointment_date FROM (
	...
	INNER JOIN patient_notifications pn ON pn.patient_id = next_appointment.patient_id
	WHERE next_appointment_date < :endDate 
		AND pn.notification_type = 'DEFAULTERS_FOR_FIVE_DAYS' 
		AND pn.notification_status = 'SENT'
		AND DATEDIFF( :endDate, pn.notification_sent_date) = 3
