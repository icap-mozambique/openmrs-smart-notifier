select coorte12meses_final.patient_id, identifier.identifier, coorte12meses_final.data_inicio, contact.value, coorte12meses_final.data_usar                                                                               
            ...
            (select inicio.*,                                                                                                                    
             ...
             INNER JOIN
			 (
			 	SELECT last_pickup_drug.patient_id, last_pickup_drug.last_pick_drug_date, 
				(
					SELECT MAX(o.value_datetime) before_last_pick_drug_date FROM patient p
						INNER JOIN encounter e ON e.patient_id = p.patient_id
						INNER JOIN obs o ON o.encounter_id = e.encounter_id
						WHERE p.voided = 0 AND e.voided = 0 AND o.voided = 0
						AND e.encounter_type = 52 AND o.concept_id = 23866 
						AND e.location_id = :location AND o.value_datetime < last_pickup_drug.last_pick_drug_date
						AND p.patient_id = last_pickup_drug.patient_id
					
					UNION
					
					SELECT MAX(e.encounter_datetime) before_last_pick_drug_date FROM patient p
						INNER JOIN encounter e ON e.patient_id = p.patient_id
						WHERE p.voided = 0 AND e.voided = 0
						AND e.encounter_type = 18 AND e.location_id = :location
						AND p.patient_id = last_pickup_drug.patient_id AND e.encounter_datetime < last_pickup_drug.last_pick_drug_date

					ORDER BY before_last_pick_drug_date DESC LIMIT 1
				)before_last_pick_drug_date,
				(
					SELECT MAX(e.encounter_datetime) before_last_encounter_date FROM patient p
							INNER JOIN encounter e ON e.patient_id = p.patient_id
							WHERE p.voided = 0 AND e.voided = 0
							AND e.encounter_type IN (6,9,53) AND e.location_id = :location
							AND p.patient_id = last_pickup_drug.patient_id AND e.encounter_datetime = before_last_pick_drug_date
				)before_last_encounter_date FROM
					(
						SELECT patient_id, MAX(last_pick_drug_date) last_pick_drug_date FROM
						(
							SELECT p.patient_id, MAX(e.encounter_datetime) last_pick_drug_date FROM patient p
								INNER JOIN encounter e ON e.patient_id = p.patient_id
								WHERE p.voided = 0 AND e.voided = 0
								AND e.encounter_type = 18 AND e.location_id = :location
							...
							SELECT p.patient_id, MAX(o.value_datetime) last_pick_drug_date FROM patient p
								INNER JOIN encounter e ON e.patient_id = p.patient_id
								INNER JOIN obs o ON o.encounter_id = e.encounter_id
								WHERE p.voided = 0 AND e.voided = 0 AND o.voided = 0
								AND e.encounter_type = 52 AND o.concept_id = 23866 
								AND e.location_id = :location AND o.value_datetime <= :endDate
									GROUP BY p.patient_id
						)last_pick_drug GROUP BY patient_id
					)last_pickup_drug
					LEFT JOIN 
					(
						SELECT p.patient_id, MAX(e.encounter_datetime) last_encounter_date FROM patient p
							...
							AND e.encounter_datetime <= :endDate
							GROUP BY p.patient_id
					)last_encounter ON last_encounter.patient_id = last_pickup_drug.patient_id AND last_encounter.last_encounter_date = last_pickup_drug.last_pick_drug_date
					WHERE last_encounter.patient_id IS NULL
			)consecutive_pickups ON consecutive_pickups.patient_id = coorte12meses_final.patient_id
			 ...
			 AND consecutive_pickups.before_last_pick_drug_date IS NOT NULL AND consecutive_pickups.before_last_encounter_date IS NULL
			 AND DATEDIFF(data_usar, :endDate) = 15