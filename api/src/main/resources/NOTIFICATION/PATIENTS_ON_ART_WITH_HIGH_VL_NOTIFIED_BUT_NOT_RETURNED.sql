   select coorte12meses_final.patient_id, identifier.identifier, coorte12meses_final.data_inicio, contact.value, coorte12meses_final.data_usar                                                                               
             from                                                                                                                    
            ...
            (select inicio.*,                                                                                                                    
             ...
             (
                ...
                (
                    SELECT p.patient_id, MAX(e.encounter_datetime) last_viral_load_date FROM patient p
                        INNER JOIN encounter e ON e.patient_id = p.patient_id
                        INNER JOIN obs o ON o.encounter_id  = e.encounter_id
                            WHERE p.voided = 0 AND e.voided = 0 AND o.voided = 0
                            AND e.encounter_type IN (13,6,9,53,51) AND o.concept_id IN (1305,856)
                            AND e.location_id = :location
                            GROUP BY p.patient_id
                )last_viral_load_date
                INNER JOIN 
                (
                    SELECT p.patient_id, o.value_numeric last_viral_load_value, e.encounter_datetime last_viral_date FROM patient p
                        INNER JOIN encounter e ON e.patient_id = p.patient_id
                        INNER JOIN obs o ON o.encounter_id  = e.encounter_id
                            WHERE p.voided = 0 AND e.voided = 0 AND o.voided = 0
                            AND e.encounter_type IN (13,6,9,53,51) AND o.concept_id IN (1305,856)
                            AND e.location_id = :location
                            GROUP BY p.patient_id
                )last_viral_load_value ON last_viral_load_value.patient_id = last_viral_load_date.patient_id AND last_viral_load_value.last_viral_date = last_viral_load_date.last_viral_load_date
                WHERE last_viral_load_value IS NOT NULL AND last_viral_load_value > 1000
             )high_viral_load ON high_viral_load.patient_id = coorte12meses_final.patient_id
			 INNER JOIN
			 (
				SELECT pe.patient_id, pe.notification_sent_date FROM patient_notifications pe
					WHERE pe.notification_status = 'SENT'
					AND pe.notification_type = 'ON_ART_WITH_HIGH_VIRAL_LOAD'
					AND DATEDIFF( :endDate, pe.notification_sent_date) = 3
			 )notified ON notified.patient_id = coorte12meses_final.patient_id
			 INNER JOIN
			 (
				SELECT patient_id, MAX(last_encounter_date) last_encounter_date FROM 
				(
					SELECT p.patient_id, MAX(e.encounter_datetime) last_encounter_date FROM patient p
							INNER JOIN encounter e ON e.patient_id = p.patient_id
							INNER JOIN obs o ON o.encounter_id = e.encounter_id
								WHERE p.voided = 0 AND e.voided = 0 AND e.encounter_type IN (6,9,18)
								AND e.location_id = :location
									GROUP BY p.patient_id
					UNION
						
						SELECT p.patient_id, MAX(o.value_datetime) last_encounter_date FROM patient p
							INNER JOIN encounter e ON e.patient_id = p.patient_id
							INNER JOIN obs o ON o.encounter_id = e.encounter_id
								WHERE p.voided = 0 AND e.voided = 0 AND o.voided = 0
								...
								AND e.location_id = :location
									GROUP BY p.patient_id
				)last_encounter GROUP BY last_encounter.patient_id
			 )last_encounter ON last_encounter.patient_id = coorte12meses_final.patient_id
             LEFT JOIN
             (
                SELECT p.patient_id, MAX(e.encounter_datetime) last_encounter FROM patient p
                    INNER JOIN encounter e ON e.patient_id = p.patient_id
                    INNER JOIN obs o ON o.encounter_id = e.encounter_id
                        WHERE p.voided = 0 AND e.voided = 0 AND o.voided = 0
                        AND o.concept_id IN (1982, 6332) AND o.value_coded = 1065
                        AND e.encounter_type = 6 AND e.encounter_datetime <= :endDate
                        AND e.location_id = :location
                        GROUP BY p.patient_id
             )pregnant_and_breastfeeding ON pregnant_and_breastfeeding.patient_id = coorte12meses_final.patient_id
			 ...
			 AND pregnant_and_breastfeeding.patient_id IS NULL AND last_encounter.last_encounter_date < notified.notification_sent_date